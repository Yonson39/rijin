    ...
    <div class="monitorgroup">
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" :content="scope.row.promptInfo"  placement="right">
                <el-button type="text" :icon="(scope.row.devOnline === '在线') ? 'el-icon-circle-check': 'el-icon-remove'"></el-button>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column
      ...
      <el-footer class="bottomfooter" height="60px">
        <div class="addgroup">
          ...
          <el-dialog title="添加新设备" :visible.sync="devDialogFormVisible" @closed="diaClose" width="740px" center>
            ...
            <div slot="footer" class="dialog-footer">
              <el-button @click="devDialogFormVisible = false">取消</el-button>
...
export default {
  ...
  methods: {
    ...
    monitorGroupAddBt: function () {
      ...
      this.devDialogFormVisible = true
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.formAddOrModify === 0) {
            ...
            // console.log(this.isObjectValueEqual(this.form, this.formOld))
          ...
          // this.devDialogFormVisible = false
          this.loadingFlag = true
        } else {
          console.log('错误提交')
        }
    ...
    backNetMoniterListQue: function () {
      back.netMoniterListQue(this.param).then(function (response) {
        ...
        for (let i = 0; i < response.data.length; i++) {
          ...
          obj.promptInfo = response.data[i].devOnline ? '在线' : '离线'
          obj.devMac = response.data[i].mac
          obj.name = response.data[i].name
